	...
	actions: {
		async init() {
			try {
				...
				const [
					...
					claptrapClient.pools(),
					claptrapClient.incentivizedPools(),
					claptrapClient.lockableDurations(),
					claptrapClient.mintParams(),
					claptrapClient.epochs(),
					claptrapClient.poolIncentivesDistrInfo(),
					claptrapClient.epochProvisions(),
				])
		...
		async loadGauges() {
			...
			try {
				...
				this.extraGauges = await claptrapClient.extraGaugesDetails(ids)
			} catch (error) {
				console.error(error)
				throw error
			} finally {
		...
		async loadLockedGauges() {
			...
			try {
				...
				const ids = configStore.extraGaugeIds
				this.lockedExtraGagues = await claptrapClient.lockedGaugesByIds(ids)
			} catch (error) {
				console.error(error)
				throw error
			} finally {
