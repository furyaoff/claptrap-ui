...
export interface MerkleDropsState {
	...
	merkledropsDetails: FanfuryMerkledrop[]
	merkledropProofs: MerkledropProof[]
	fanfuryBlock?: BlockResponse
}
	...
	actions: {
		...
		async loadAirdrops(address?: string) {
			try {
				...
				this.fanfuryBlock = await claptrapClient.fanfuryBlocks()
				const merkledropsDetails = await claptrapClient.merkledrops(merkledropIds)
				const proofs: MerkledropProof[] = []

				for (const proof of merkledropProofs) {
					if (proof) {
						const isClaimed = await claptrapClient.merkledropClaimed(
							proof.merkledrop_id,
							proof.index
						)
	...
	getters: {
		merkledropsWithProofs: ({
			...
			return merkledrops.map((merkledrop) => {
				...
				if (fanfuryBlock) {
					const lastHeight = new Decimal(fanfuryBlock.block.header.height)
					const endHeight = new Decimal(merkledrop.end_height)
					const lastTime = new Date(fanfuryBlock.block.header.time)
