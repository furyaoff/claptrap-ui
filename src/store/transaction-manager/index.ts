	...
	actions: {
		...
		async merkledropClaim(
			...
			try {
				...
				if (window.keplr && fanfuryToken && authStore.fanfuryAddress) {
					const signer = await window.keplr.getOfflineSignerAuto(
						fanfuryToken.chainID
					)

					const manager = new TransactionManager(signer, fanfuryToken)
								...
								if (response.code === 404) {
									...
									setTimeout(() => {
										...
										if (merkledropsStore.merkledrops.length > 0) {
											merkledropsStore.loadAirdrops(authStore.fanfuryAddress)
										}
									}, 2000)
								}
