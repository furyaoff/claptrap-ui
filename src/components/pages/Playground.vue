...
const addressAlreadyRegistered = computed(() => {
	return (
		twitterStore.authors.find(
			(author) => author.address === authStore.fanfuryAddress
		) !== undefined
	)
})

const twitterLink = computed(() => {
	let messagge = `👉 #smashdatestnet of @claptrapzone, the #music #FanToken App powered by @BitSongOfficial on @osmosiszone 
 
🥁 Let’s play it!`

	if (authStore.fanfuryAddress) {
		messagge = messagge.concat(`\n${authStore.fanfuryAddress}`)
	} else {
