...
const fanfuryCoinLookup = computed(() => {
	const fanfuryToken = configStore.fanfuryToken

	if (fanfuryToken) {
		const coinLookup = fanfuryToken.coinLookup.find(
			(coin) => coin.viewDenom === fanfuryToken.symbol
		)
...
<template>
	<div>
			...
			<div class="flex column items-start col-8 col-md-5">
				<p class="fs-16 !leading-24">
					<span class="opacity-40">
						Welcome to Claptrap incentivized Testnet! Each participant might be able
						to get great rewards in FURY and FanTokens simply playing with the
						platform.
					</span>
					<a
						:href="guideUrl"
		...
		<div class="row text-weight-medium q-col-gutter-xl q-mb-60">
			...
			<div class="col-8 !w-md-1/3">
				<CardWithIcon header="2. Transfer your FURY">
					<p class="fs-14 !leading-24 opacity-50 q-pr-32">
						Transfer FURY from the Osmosis testnet chain to the BitSong testnet chain.
					</p>
			...
			<div class="col-8 !w-md-1/3">
				<CardWithIcon header="3. You are a winner if">
					<p class="fs-14 !leading-24 opacity-50 q-pr-32">
						Your final balance is greater than the initial 10,000 FURY.
					</p>
		...
		>
			...
			<template v-slot:body-cell-balance="slotProps">
				<q-td :props="slotProps">
					<p
						class="text-center"
						v-if="configStore.fanfuryToken && fanfuryCoinLookup"
						:class="{
							'opacity-20': !slotProps.row.valid || slotProps.row.disqualified,
						}"
					>
			...
			<template v-slot:body-cell-reward="slotProps">
				<q-td :props="slotProps">
					...
					<p
						class="text-center"
						v-else-if="configStore.fanfuryToken && fanfuryCoinLookup"
					>
						{{
							balancedCurrencyFixed(
								distributionAmount(slotProps.row.balance.amount),
								3
							)
						}}
					</p>
